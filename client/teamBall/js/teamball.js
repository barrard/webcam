 // var TeamBallApp = new Vue({
 // 	el:'#Index',
 // 	data:{
 // 		teamName:'VUE JS BITCH!!'
 // 	}
 // })

 // TeamBallApp.teamName = 'This is databinded'
 ...
 console.log('TeamBall App.js')
//establish socket and request the Teamball login/register page
 // socket.on('loginForm', function (socketId) {
 // 	socket.emit('teamBallPlease', socketId)
socket.on('teamBallConection', function(message){
	console.log(message)
})
 // });
//catch the loginRegister page and display in the Main content
 socket.on('mainHTMLcontent', function(data){
 	console.log('loginRegister content')
 	// console.log(data)

 	// $('.mainContent').html(data);
 	$('.mainContent').append('<loginRegister></loginRegister>')
 });

 // socket.on('mainContent', function(data){
 // 	console.log('we got to render the HTML in mainContent')
 // 	$('.mainContent').html(data)
 // })
...
function getTeamInfo(val){
	var oponentName = $('#opponentName')
	$(opponentName).html(val)
	$.ajax({
		type:'GET',
		url:'/getTeamInfo/'+val,
		success:function(result){
			...
			$('#opponentInfo').addClass('bounceInRight').one('animationend',
				function() {
				   $(this).html(result.teamInfo)
				})
			$('#resultHistory').addClass('bounceInUp').one('animationend',
				function() {
					if(Array.isArray(result.teamHistory)){
				   $(this).html('result.teamHistory[0]')
				}else{
					console.log(opponentName)
					$(this).html('No games have been played between '+teamName+' and '+val)
	...
	// // var teamName = val;
	// var url = '/getTeamInfo/'+val;

	// xhr.onreadystatechange = function(){
	// 	if (xhr.readyState==4&&xhr.status==200) {
	// 		$('#opponentInfo').addClass('bounceInRight').one('animationend',
	// 			function() {
	// 			   $(this).html(xhr.responseText.teamInfo)
	// 			})
	// 		$('#resultHistory').addClass('bounceInUp').one('animationend',
	// 			function() {
	// 			   $(this).html('Game istory')
	// 			})
